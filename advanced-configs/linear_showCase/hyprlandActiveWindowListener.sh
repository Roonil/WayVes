...
looper() {
    ...
    lastSwitch=1
      ...
      WayVes -c merged_bars -v1&
      # WayVes -c linear_boxes -v1&
      WayVes -c linear_lights -v1& 
    ...
    then
      ...
      WayVes -c linear_waves -w 900 -r -1 -d 0 -u 1 -t -40 && echo -e "isSmall=0" > /tmp/WayVes/linear_waves&
      WayVes -c linear_holder -w 120 -h 100 -r 1020 -d 0 -u 1 -t -65&&  echo -e "visualiserMode = 0\nparticleRadius = 12" > /tmp/WayVes/linear_holder &
      WayVes -c linear_boxes -t -40 -l 720 -w 90 -h 90 -z 2 &&  echo -e "isLarge=0" > /tmp/WayVes/linear_boxes&

      WayVes -c merged_bars -v0&
      # WayVes -c linear_boxes -v0&
      WayVes -c linear_lights -v0&
   fi


}

# while true; do looper; done;
